...
Inductive ExpEq : Ctx -> Exp -> Exp -> Typ -> Prop := 
  ...
  | exp_eq_beta_rec_zero : forall Γ tz ts T,
    Γ ⊢ tz : T ->
    Γ ⊢ ts : ℕ → T → T ->
    Γ ⊢ exp_rec tz ts exp_zero ≈ tz : T
  | exp_eq_beta_rec_suc : forall Γ tz ts tn T,
    Γ ⊢ tz : T ->
    Γ ⊢ ts : ℕ → T → T ->
    Γ ⊢ tn : ℕ ->
    Γ ⊢ exp_rec tz ts (exp_suc tn) ≈ exp_app (exp_app ts tn) (exp_rec tz ts tn) : T
  ...
  | exp_eq_prop_rec : forall Γ Δ σ tz ts tn T,
    ...
    Δ ⊢ ts : ℕ → T → T ->
    Δ ⊢ tn : ℕ ->
    Γ ⊢ exp_subst (exp_rec tz ts tn) σ ≈ exp_rec (exp_subst tz σ) (exp_subst ts σ) (exp_subst tn σ) : T
  ...
  | exp_eq_comp_rec : forall Γ tz tz' ts ts' tn tn' T,
    ...
    Γ ⊢ ts ≈ ts' : ℕ → T → T ->
    Γ ⊢ tn ≈ tn' : ℕ ->
    Γ ⊢ exp_rec tz ts tn ≈ exp_rec ts' ts' tn' : T 
